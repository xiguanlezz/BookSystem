...
<div class="address">
    <div class="wrapper clearfix">
        <a href="${pageContext.request.contextPath}/indexselect">首页</a><span>/</span>
        <a href="selectproductlist?id=${fcate.CATE_ID}">${fcate.CATE_NAME}</a><span>/</span>
        <a href="selectproductlist?fid=${fcate.CATE_ID}&cid=${ccate.CATE_ID}" class="on">${ccate.CATE_NAME}</a>
    </div>
</div><!-------------------current---------------------->
<div class="current">
    <div class="wrapper clearfix">
        <h3 class="fl">
            <c:if test="${requestScope.ccate.CATE_ID!=null}">
                ${requestScope.ccate.CATE_NAME}
            </c:if>
            <c:if test="${requestScope.ccate.CATE_ID==null}">
                ${requestScope.fcate.CATE_NAME}
            </c:if>
        </h3>
...
<ul class="proList wrapper clearfix">
        ...
        <c:forEach items="${requestScope.ps}" var="singleP">
            <li>
                <a href="selectproductview?id=${singleP.PRODUCT_ID}">
                    <dl>
                        <dt><img src="images/product/${singleP.PRODUCT_FILENAME}" alt="..."></dt>
                        <dd>${singleP.PRODUCT_NAME}</dd>
                        <dd>￥${singleP.PRODUCT_PRICE}.00</dd>
                    </dl>
                </a>
            </li>
        </c:forEach>
<%--        <c:if test="${ccate}"></c:if>--%>
